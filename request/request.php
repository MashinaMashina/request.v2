...
Author:	http://t-do.ru/MashinaMashina
		http://t.me/MashinaMashina
...
class request {
	...
	private $out_headers = array();
	private $options = array();
	private $data = array();
	private $session = false;
	private $dump_text = '';
	private $charset = false;
	private $user_agent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.181 Safari/537.36';
	...
	public function __construct($url)
	{
		...
		$this->set(CURLOPT_USERAGENT, $this->user_agent);
		$this->set(CURLOPT_HEADERFUNCTION, array(&$this, '_set_headers'));
		$this->set(CURLOPT_CONNECTTIMEOUT, 4);
		$this->set(CURLOPT_TIMEOUT, 8);
		$this->set(CURLOPT_AUTOREFERER, true);
		$this->set(CURLOPT_ENCODING, 'gzip');
		
		$this->set_headers('Accept', '*/*');
		$this->set_headers('Accept-Encoding', 'gzip, deflate, br');
		$this->set_headers('Accept-Language', 'ru-RU,ru;q=0.9,en-US;q=0.8,en;q=0.7');
	}
	...
	{
		$this->curl_options = $session->curl_opt + $this->curl_options;
		$this->request_options = $session->request_opt + $this->request_options;
		
		$this->out_headers = array_merge_recursive($this->out_headers, $session->out_headers);

		$this->curl_options[CURLOPT_HTTPHEADER] = $this->out_headers;
		
		$cookie_file = $session->get_cookie_file();
		...
		$this->set(CURLOPT_USERAGENT, $user_agent);
	}
	...
	public function send()
	{
		...
		if( $this->request_options['convert_encoding'] and !empty($this->response))
		{
			...
			if( strtoupper($charset) !== 'UTF-8')
			{
				try
				{
					$response = iconv($charset, 'UTF-8//IGNORE', $this->response);
					
					if( $response)
					{
						$this->dump_text .= "Encoded from {$charset} to UTF-8<br />\n";
						
						$this->response = $response;
					}
					else
					{
						$this->dump_text .= "Cant encode from {$charset} to UTF-8<br />\n";
					}
				} catch (Exception $e) {
					$this->dump_text .= "Cant encode from {$charset} to UTF-8; ".$e->getMessage()."<br />\n";
				}
			}
		}
	...
	public function get_charset()
	{
		...
		if( count($match))
		{
			$this->dump_text .= "Charset from headers\r\n";
		...
		if( count($match))
		{
			$this->dump_text .= "Charset from HTML body\r\n";
			$this->charset = $match[3];
			return $this->charset;
		}
		
		$this->dump_text .= "Charset from mb_detect_encoding()\r\n";
	...
	public function dump()
	{
		...
		{
			if( !isset($this->opt_codes[$k])) $this->opt_codes[$k] = "(undefined; code: {$k})";
		...
		$safe_response = htmlspecialchars($this->response);
		
		if( empty($safe_response) and !empty($this->response))
		{
			$safe_response = '<i>(cant view response)</i>';
		}
		else
		{
			$safe_response = "<pre>{$safe_response}</pre>";
		}
